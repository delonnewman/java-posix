...
# RHEL 4
CXXFLAGS = -O -pthread -I/java/include -I/java/include/linux -DHAVE_UTIMES \
	-DHAS_SEM_POST -DHAVE_MSGBUF -Dsigthreadmask=pthread_sigmask
LIB =	-pthread -L/java/jre/lib/i386/ -ljava -lc
JAVALIB = /java/jre/lib/i386/libposix.so
...
posix.so: $(POSIX_OBJS)
	g++ -shared -o posix.so $(POSIX_OBJS) $(LIB)
